...
.keyword-order-controls{display:flex;flex-direction:column;gap:2px;margin-right:8px}
.order-btn{background:#6c757d;color:white;border:none;padding:2px 6px;border-radius:3px;cursor:pointer;font-size:10px;line-height:1;transition:all 0.2s}
.order-btn:hover{background:#495057}
...
.product-order-controls{display:flex;flex-direction:column;gap:2px;margin-right:5px}
...
// 키워드 순서 변경 함수들
function moveKeywordUp(keywordIndex){if(keywordIndex<=0)return;const keyword=kw.splice(keywordIndex,1)[0];kw.splice(keywordIndex-1,0,keyword);if(cKI===keywordIndex)cKI=keywordIndex-1;else if(cKI===keywordIndex-1)cKI=keywordIndex;updateUI();showSuccessModal('순서 변경','키워드 순서가 위로 이동되었습니다.','⬆️');}
function moveKeywordDown(keywordIndex){if(keywordIndex>=kw.length-1)return;const keyword=kw.splice(keywordIndex,1)[0];kw.splice(keywordIndex+1,0,keyword);if(cKI===keywordIndex)cKI=keywordIndex+1;else if(cKI===keywordIndex+1)cKI=keywordIndex;updateUI();showSuccessModal('순서 변경','키워드 순서가 아래로 이동되었습니다.','⬇️');}
// 상품 순서 변경 함수들
function moveProductUp(keywordIndex,productIndex){if(productIndex<=0)return;const product=kw[keywordIndex].products.splice(productIndex,1)[0];kw[keywordIndex].products.splice(productIndex-1,0,product);if(cKI===keywordIndex&&cPI===productIndex)cPI=productIndex-1;else if(cKI===keywordIndex&&cPI===productIndex-1)cPI=productIndex;updateUI();showSuccessModal('순서 변경','상품 순서가 위로 이동되었습니다.','⬆️');}
function moveProductDown(keywordIndex,productIndex){if(productIndex>=kw[keywordIndex].products.length-1)return;const product=kw[keywordIndex].products.splice(productIndex,1)[0];kw[keywordIndex].products.splice(productIndex+1,0,product);if(cKI===keywordIndex&&cPI===productIndex)cPI=productIndex+1;else if(cKI===keywordIndex&&cPI===productIndex+1)cPI=productIndex;updateUI();showSuccessModal('순서 변경','상품 순서가 아래로 이동되었습니다.','⬇️');}
...
function updateProductsList(){const l=document.getElementById('productsList');if(kw.length===0){l.innerHTML='<div class="empty-state"><h3>📦 상품이 없습니다</h3><p>위의 "키워드 추가" 버튼을 클릭하여<br>첫 번째 키워드를 추가해보세요!</p></div>';return;}let h='';kw.forEach((k,ki)=>{h+=`<div class="keyword-group draggable" draggable="true" data-keyword-index="${ki}"><div class="keyword-header"><div class="keyword-info"><span class="keyword-name">📁 ${k.name}</span><span class="product-count">${k.products.length}개</span></div><div class="keyword-actions"><div class="keyword-order-controls"><button type="button" class="order-btn" onclick="moveKeywordUp(${ki})" ${ki===0?'disabled':''}title="키워드 위로">▲</button><button type="button" class="order-btn" onclick="moveKeywordDown(${ki})" ${ki===kw.length-1?'disabled':''}title="키워드 아래로">▼</button></div><button type="button" class="btn btn-success btn-small" onclick="addProduct(${ki})">+상품</button><button type="button" class="btn btn-danger btn-small" onclick="deleteKeyword(${ki})">🗑️</button></div></div>`;k.products.forEach((p,pi)=>{const si=getStatusIcon(p.status,p.isSaved);h+=`<div class="product-item draggable" draggable="true" data-keyword="${ki}" data-product="${pi}" onclick="selectProduct(${ki},${pi})"><span class="product-status">${si}</span><span class="product-name">${p.name}</span><div class="product-actions"><div class="product-order-controls"><button type="button" class="order-btn" onclick="event.stopPropagation();moveProductUp(${ki},${pi})" ${pi===0?'disabled':''}title="상품 위로">▲</button><button type="button" class="order-btn" onclick="event.stopPropagation();moveProductDown(${ki},${pi})" ${pi===k.products.length-1?'disabled':''}title="상품 아래로">▼</button></div><button type="button" class="btn btn-danger btn-small" onclick="event.stopPropagation();deleteProduct(${ki},${pi})">🗑️</button></div></div>`;});h+='</div>';});l.innerHTML=h;setupDragAndDrop();}
