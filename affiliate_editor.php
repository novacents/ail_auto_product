...
function processExcelData(data){if(!data||data.length===0){showDetailedError('데이터 오류','엑셀 파일에서 유효한 데이터를 찾을 수 없습니다.');return;}data.forEach(item=>{if(item.keyword&&item.url&&item.product_detail){let keywordIndex=-1;for(let i=0;i<kw.length;i++){if(kw[i].name===item.keyword){keywordIndex=i;break;}}if(keywordIndex===-1){kw.push({name:item.keyword,products:[]});keywordIndex=kw.length-1;}const product={id:Date.now()+Math.random(),url:item.url,name:item.product_detail||`상품 ${kw[keywordIndex].products.length+1}`,status:'empty',analysisData:null,userData:{},isSaved:false,generatedHtml:null};kw[keywordIndex].products.push(product);}});updateUI();}
