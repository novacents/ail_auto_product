...
function updateProductsList(){const l=document.getElementById('productsList');if(kw.length===0){l.innerHTML='<div class="empty-state"><h3>📦 상품이 없습니다</h3><p>위의 "키워드 추가" 버튼을 클릭하여<br>첫 번째 키워드를 추가해보세요!</p></div>';return;}let h='';kw.forEach((k,ki)=>{h+=`<div class="keyword-group draggable" draggable="true" data-keyword-index="${ki}"><div class="keyword-header"><div class="keyword-header-left"><div class="keyword-order-controls"><button type="button" class="keyword-order-btn" onclick="moveKeywordUp(${ki})" ${ki===0?'disabled':''}title="키워드 위로">▲</button><button type="button" class="keyword-order-btn" onclick="moveKeywordDown(${ki})" ${ki===kw.length-1?'disabled':''}title="키워드 아래로">▼</button></div><div class="keyword-info"><span class="keyword-name">📁 ${k.name}</span><span class="product-count">${k.products.length}개</span></div></div><div class="keyword-actions"><button type="button" class="btn btn-success btn-small" onclick="addProduct(${ki})">+상품</button><button type="button" class="btn btn-danger btn-small" onclick="deleteKeyword(${ki})">🗑️</button></div></div>`;k.products.forEach((p,pi)=>{const si=getStatusIcon(p.status,p.isSaved);h+=`<div class="product-item draggable" draggable="true" data-keyword="${ki}" data-product="${pi}" onclick="selectProduct(${ki},${pi})"><div class="product-order-controls"><button type="button" class="product-order-btn" onclick="event.stopPropagation();moveProductUp(${ki},${pi})" ${pi===0?'disabled':''}title="상품 위로">▲</button><button type="button" class="product-order-btn" onclick="event.stopPropagation();moveProductDown(${ki},${pi})" ${pi===k.products.length-1?'disabled':''}title="상품 아래로">▼</button></div><span class="product-status">${si}</span><span class="product-name">${p.name}</span><div class="product-actions"><button type="button" class="btn btn-danger btn-small" onclick="event.stopPropagation();deleteProduct(${ki},${pi})">🗑️</button></div></div>`;});h+='</div>';});l.innerHTML=h;setupDragAndDrop();}
